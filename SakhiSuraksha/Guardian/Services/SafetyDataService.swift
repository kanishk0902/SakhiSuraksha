...
/// Concrete RouteSafetyProvider built on real (currently sparse) incident/CCTV
/// data. Filters SafetyDataService's incidents/CCTV to those near the given
/// route, then reuses JourneyService's scoring formula. Honest: with
/// incidents/cctv empty (no data source yet), every route scores from the same
/// baseline — this never fabricates a difference between routes to make one
/// look artificially "safer."
struct LocalRouteSafetyProvider: RouteSafetyProvider {
    let data: SafetyDataService
    /// How close an incident/CCTV point must be to a route coordinate to count
    /// as "near" that route.
    var corridorRadiusMeters: Double = 120

    func safetyScore(for route: [CLLocationCoordinate2D]) async throws -> Int {
        guard !route.isEmpty else { return 50 }
        ...
        return JourneyService.routeSafetyScore(incidents: nearIncidents, cctv: nearCCTV)
    }
}
