...
//  Guardian
//
//  Detects three sharp wrist/phone flicks in quick succession using
//  CoreMotion — the iPhone-side equivalent of GuardianWatch's gesture SOS,
//  for use without an Apple Watch. Foreground-only: iOS suspends motion
//  updates once the app leaves the active/foreground state, matching the
//  same honest limitation already documented for Discreet SOS voice
//  detection — this is armed only while Guardian is open, not a true
//  always-on background trigger.
//
...
final class GestureTriggerService {
    ...
    /// Three flicks must land within this window of each other to count as
    /// one gesture rather than unrelated motion.
    ...
    func arm(onFlick: @escaping () -> Void, onTripleFlick: @escaping () -> Void) {
        guard motionManager.isDeviceMotionAvailable, !isArmed else { return }
    ...
    /// — used while a countdown is in progress so a false-positive flick
    /// can't fire mid-countdown, then resumed on cancel.
    func pause() {
        motionManager.stopDeviceMotionUpdates()
    }
